    ...
    public class FCSFile3_2 : IFCSFile
    {
        ...
        protected override IList<Measurement> AnalyseMeasurements(Dictionary<string, string> textSegment, uint par, DataType defaultDataType)
        {
            ...
            if (defaultDataType == DataType.F || defaultDataType == DataType.D)
                foreach (var item in measurements) item.Gain = 1d;
            return measurements;
        }
        /// <summary>
        /// 解析补偿
        ...
        {
            if (fcs.TextSegment.ContainsKey(Keys.SpilLoverKey)) fcs.Compensation = new Compensation(fcs.TextSegment[Keys.SpilLoverKey]);
        }
        #endregion
        ...
        protected override void ResetCompensation(FCS fcs)
        {
            ...
            fcs.TextSegment[Keys.SpilLoverKey] = temp;
        }
        #endregion
    }
}
